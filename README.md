# RSIS 
 ...
 RSIS是一个面向 ArcGIS 遥感影像的训练数据切片工具。它将遥感栅格影像与面要素标注配对处理，生成可直接用于语义分割数据集制作的影像切片和标签切片。输出原始的xxx.tif影像文件和与其对应的面要素文件xxx.shapefile，其可以自动将影像按设定的大小和步长完成切片。并且生成文件名一一对应的label-image数据集。以适配不同模型进行深度学习训练
